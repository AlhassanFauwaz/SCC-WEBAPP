...
export class CaseFilterService {
    ...
    private static matchesKeyword(caseData: Case, keyword: string): boolean {
        ...
        return (
            ...
            (caseData.description ? caseData.description.toLowerCase().includes(searchTerm) : false) ||
            (caseData.citation ? caseData.citation.toLowerCase().includes(searchTerm) : false) ||
            (caseData.court ? caseData.court.toLowerCase().includes(searchTerm) : false)
        );
    }
    ...
    private static matchesCaseType(caseData: Case, caseType: string): boolean {
        ...
        return (
            ...
            (caseData.description ? caseData.description.toLowerCase().includes(searchType) : false) ||
            (caseData.citation ? caseData.citation.toLowerCase().includes(searchType) : false)
        );
    }
